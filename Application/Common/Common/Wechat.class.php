...
use Common\Common\RedisStorage;
...
class Wechat
{
	...
	const USER_INFO_URL_BATCHET='/user/info/batchget?';
	...
	const GET_ALL_PRIVATE_TEMPLATE = '/cgi-bin/template/get_all_private_template?'; //模板消息列表
	...
	const ZXCS_BUSINESS_WECHAT_ACCESS_TOKEN = 'laravel:wechatAccessToken';//微信access_token缓存key
	const ZXCS_BUSINESS_WECHAT_JSAPI_TICKET= 'laravel:wechatJsapiTicket';//微信jsapi_ticket缓存key
	const ZXCS_BUSINESS_WECHAT_API_TICKET_WXCARD= 'laravel:wechatApiTticketWxcard';//微信api_ticket_wxcard缓存key
     ...
     * @return mixed
     */
    public function log($log, $type = 1){
    		if ($this->debug && ($this->logcallback)) {
    			if (is_array($log)) $log = print_r($log,true);
    			return call_user_func_array($this->logcallback,[$log, $type]);
    		}
    }
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
        ...
        $this->redis->setString($cachename, serialize($value), $expired);
		return true;
	}
	...
	protected function getCache($cachename){
        ...
        if($this->redis->checkString($cachename)) {
            return  unserialize($this->redis->getString($cachename));
        }
		return false;
	}
...
//		$authname = 'wechat_access_token'.$appid;
		$authname = self::ZXCS_BUSINESS_WECHAT_ACCESS_TOKEN.$appid;
	...
	public function resetJsTicket($appid=''){
		...
		$authname = self::ZXCS_BUSINESS_WECHAT_JSAPI_TICKET.$appid;
		$this->removeCache($authname);
		return true;
	}
...
//		$authname = 'wechat_api_ticket_wxcard'.$appid;
		$authname = self::ZXCS_BUSINESS_WECHAT_API_TICKET_WXCARD.$appid;
		if ($rs = $this->getCache($authname))  {
			$this->api_ticket = $rs;
			return $rs;
		}
	 ...
	 * @return boolean | array('ticket'=>'qrcode字串','expire_seconds'=>2592000,'url'=>'二维码图片解析后的地址')
	 */
     ...
     */
    public function getUserInfoList($userlist){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::USER_INFO_URL_BATCHET.'access_token='.$this->access_token,self::json_encode($userlist));
		return $this->handleResult($result);
    }
     ...
     * @param int $latitude 纬度坐标，与经度同时传入；与城市二选一传入
     * @param int $longitude 经度坐标，与纬度同时传入；与城市二选一传入
...
*/
   	...
   	{
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::GET_ALL_PRIVATE_TEMPLATE . 'access_token=' . $this->access_token);
        return $this->handleResult($result);
    }
    ...
    private function handleResult($result)
    {
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
	            $this->log($result, 0);
	            if (++$this->errTimes <= 3) {
	            	if ($json['errcode'] == '40001') {	//access_token 失效
	            		$this->resetAuth();
	            		$this->checkAuth();
	            		$fun = array_shift(debug_backtrace());
	            		return call_user_func_array([$fun['class'], $fun['function']], $fun['args']);
	            	}
	            };
